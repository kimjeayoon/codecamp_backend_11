...
interface IProfile {
    ...
    age:  number | string
    school: string
    hobby?: string // 있어도되고 없어도 되는것을 의미한다
}
...
function add(num1: number, num2: number, unit:string): string { // 함수는 타입추론이 안된다 
    return num1 + num2 + unit
}

const result =  add( 1000, 2000, "원") // 결과의 리턴 타입도 예측 가능 !!!

const add2 = (num1: number, num2: number, unit:string): string => { // 함수는 타입추론이 안된다 
    return num1 + num2 + unit
}
